...
import SearchItem from "../../../components/booking_components/searchItem/SearchItem";
...
const List = () => {
  ...
  useEffect(() => {
    fetchData();
  }, [destination, dates, min, max]);

  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `http://localhost:8800/api/hotels/getHotels?city=${destination}&min=${min || 0}&max=${max || 999}`
      );
